...
public class AuthService : IAuthService
{
    ...
    public async Task<DataResult<Models.Response.UserProfileDto>> GetUserProfileInfo()
    {
        var userId = _claimHelper.GetUserId();

        var user = await _unitOfWork.Users.FirstOrDefaultAsync(u => u.Id == userId);

        var profileDto = _mapperHelper.Map<Models.Response.UserProfileDto>(user);

        return new DataResult<Models.Response.UserProfileDto>(data: profileDto, status: ResultStatus.Ok);
    }
